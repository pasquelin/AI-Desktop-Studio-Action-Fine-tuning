...
function tone(frequency: number): Buffer {
  ...
  for (let sample = 0; sample < dataBytes / 2; sample++)
    wav.writeInt16LE(
      Math.round(Math.sin((2 * Math.PI * frequency * sample) / sampleRate) * 8192),
      44 + sample * 2,
    )
  return wav
}
...
function triangle(): Buffer {
  ...
  for (const [index, value] of positions.entries()) binary.writeFloatLE(value, index * 4)
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  await runCheck(
    async () => {
      ...
      const directory = join(import.meta.dirname, '../artifacts/fixtures/media')
