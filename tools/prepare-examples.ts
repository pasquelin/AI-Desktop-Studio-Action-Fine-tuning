...
await runCheck(
  async () => {
    ...
    for (const action of catalogue.actions) {
      if (!record(action) || typeof action.name !== "string")
    ...
    const links = await readJson(
      join(root, "datasets/scenarios/action-bench-links.json"),
    );
    if (!record(links)) throw new Error("Invalid bench links");
    const bench = (
      await readFile(
        join(root, "datasets/scenarios/studio-bench.jsonl"),
        "utf8",
      )
    )
      .trim()
      ...
      .map((line): BenchExample => {
        ...
        for (const key of [
          ...
          if (typeof value[key] !== "string")
            throw new Error(`Invalid bench ${key}`);
    ...
    const cases = inventory.cases.map((item) => {
      ...
      if (
        ...
        !ranks.every(
          (rank): rank is string =>
            typeof rank === "string" && knownRanks.has(rank),
        )
      )
    ...
    const summary = {
      ...
      schemaRejectedVariants: variants.filter(
        (item) => !item.expectedWireResult.accepted,
      ).length,
      schemaAcceptedVariants: variants.filter(
        (item) => item.expectedWireResult.accepted,
      ).length,
      ...
      benchSnapshotHashes: Object.fromEntries(
        bench.map((item) => [item.source, item.sourceHash]),
      ),
