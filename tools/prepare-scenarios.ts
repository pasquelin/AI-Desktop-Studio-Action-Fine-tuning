...
await runCheck(
  async () => {
    ...
    for (const name of actionMap.keys())
      if (!covered.has(name))
        throw new Error(`Action has no scenario: ${name}`);
    const output = join(root, "artifacts/scenarios");
    await mkdir(output, { recursive: true });
    const authored = join(root, "datasets/scenarios");
    const bindings = JSON.parse(
      await readFile(join(authored, "bindings.json"), "utf8"),
    );
    const french = JSON.parse(
      await readFile(join(authored, "templates.fr.json"), "utf8"),
    );
    const benchLinks = JSON.parse(
      await readFile(join(authored, "action-bench-links.json"), "utf8"),
    );
    const bench = (await readFile(join(authored, "studio-bench.jsonl"), "utf8"))
      .trim()
      .split("\n")
      .map((line) => JSON.parse(line));
    const journeyFrench = JSON.parse(
      await readFile(join(authored, "journeys.fr.json"), "utf8"),
    );
    ...
    for (const language of languages) {
      for (const [name, source, translated] of [
        ...
        if (errors.length)
          throw new Error(`${language}/${name}: ${errors.join("; ")}`);
      }
    }
    ...
    for (const item of inventory.cases) {
      ...
      if (!record(action.inputSchema))
        throw new Error(`Missing action schema: ${item.action}`);
      ...
      if (!references) {
        ...
        references = bench.filter((entry: { rank: string }) =>
      ...
      const localized = Object.fromEntries(
        languages.map((language) => {
          ...
          if (
            ...
            throw new Error(
              `Missing action translation: ${item.action}/${language}`,
            );
    ...
    const journeys = inventory.journeys.map((item) => {
      ...
      for (const name of names)
        if (!actionMap.has(name))
          throw new Error(`Unknown journey action ${name}`);
      const ranks = new Set(names.flatMap((name) => benchLinks[name] ?? []));
      const references = bench.filter((entry: { rank: string }) =>
        ranks.has(entry.rank),
      );
      return {
        ...item,
        ...
        expectedResult:
          item.specification.match(/^- Contrôle : (.*)$/m)?.[1] ??
          "Missing authored control",
        oracle: {
