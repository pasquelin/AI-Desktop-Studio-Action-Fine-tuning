...
function describeChanges(sourceRoot: string, from: string, to: string): string {
  let files: string[];
  try {
    files = execGit(sourceRoot, "diff", "--name-only", from, to)
      .split("\n")
      .filter(Boolean);
  } catch {
...
await runCheck(
  () => {
    const saved: unknown = JSON.parse(
      readFileSync(resolve(root, "configs/freshness.json"), "utf8"),
    );
    if (!valid(saved))
      throw new Error("Invalid scenario/training freshness reference.");
    ...
    if (saved.trainedRevision === null)
      console.log(
        "Entraînement : aucun modèle entraîné enregistré pour le moment.",
      );
    if (saved.scenariosRevision !== current)
      console.warn(
        describeChanges(link.sourceRoot, saved.scenariosRevision, current),
      );
    return [];
  },
  () =>
