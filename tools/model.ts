...
await runCheck(
  async () => {
    if (
      ...
      !["pull", "check", "evaluate"].includes(mode ?? "")
    )
    ...
    const installed = models.find(
      (item) => record(item) && item.name === MODEL,
    );
    ...
    for (const item of modelCases) {
      ...
      console.log(
        `${item.id}: ${verdict.passed ? "PASS" : "FAIL"} — ${verdict.reason}`,
      );
      await writeFile(
    ...
    console.log(
      `Result: ${passed}/${results.length}; report: ${join(directory, "baseline.md")}`,
    );
    return [];
  },
