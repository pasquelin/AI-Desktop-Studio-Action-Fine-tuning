...
await runCheck(
  async () => {
    const source = await readFile(join(root, 'datasets/scenarios/templates.fr.json'), 'utf8')
    const templates: unknown = JSON.parse(source)
    if (!record(templates) || Object.values(templates).some(value => typeof value !== 'string'))
    ...
    for (const [language, name] of Object.entries(languages)) {
      ...
      try {
        console.log(`${language}: drafting ${Object.keys(templates).length} scenario instructions`)
        ...
        if (!record(translated) || Object.keys(translated).length !== Object.keys(templates).length)
          throw new Error(`Invalid translation keys: ${language}`)
        await mkdir(join(root, 'artifacts/scenarios'), { recursive: true })
        await writeFile(
          join(root, 'artifacts/scenarios', `locale-candidate-${language}.json`),
        ...
        console.log(`${language}: written; placeholders checked; semantic review still required`)
      } catch (error) {
        const message = error instanceof Error ? error.message : `Translation failed: ${language}`
