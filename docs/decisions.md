...
2. **Multilingue mondial.** Quinze langues du catalogue Studio pour le premier essai. Comparer formulations, états et ambiguïtés ; ne pas confondre langues annoncées et fiabilité des actions.
...
## Essai du modèle autorisé

Le lot de propositions locales utilise Qwen3.5-2B via Ollama déjà installé, en Q8_0 (environ 2,7 Go de téléchargement). Ce choix sert à mesurer la base avant entraînement ; il ne décide pas du moteur de fine-tuning ni du format embarqué final. Voir `model-usage.md`.
