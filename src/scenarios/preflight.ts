...
function deferredPaths(value: unknown, path = ''): string[] {
  if (Array.isArray(value))
    return value.flatMap((item, index) => deferredPaths(item, `${path}/${index}`))
...
function relevant(error: ErrorObject, deferred: string[]): boolean {
  ...
  if (
    ...
    throw new Error('Cannot preflight a conditional schema with unresolved descendants')
  return true
}

const ajv = new Ajv({ strict: false, allErrors: true })
const catalogueValidators = new WeakMap<object, Map<string, ReturnType<Ajv['compile']>>>()

/** Root compilation preserves local $defs/$ref resolution; unsupported references fail closed. */
export function checkScenarioInputs(plan: DeclarativeScenario, tools: unknown): void {
  ...
  if (!validators) {
    ...
    for (const tool of tools) {
      ...
      if (validators.has(tool.name)) throw new Error(`Duplicate MCP tool: ${tool.name}`)
      try {
        validators.set(tool.name, ajv.compile(tool.inputSchema))
      } catch (error) {
        throw new Error(`Cannot preflight MCP schema ${tool.name}: ${String(error)}`)
      }
    }
  ...
  for (const step of plan.steps) {
    ...
    if (!validate) throw new Error(`${plan.id}/${step.id}: unknown action ${step.action}`)
    const deferred = deferredPaths(step.input)
    if (validate(step.input)) continue
    const errors = (validate.errors ?? []).filter(error => relevant(error, deferred))
    if (errors.length)
      throw new Error(
