...
export function compileCaseExample(
  ...
  if (!record(action.inputSchema)) throw new Error(`Missing internal schema: ${item.action}`)
  ...
  const variants = design.fixture.requests.map(({ label, input }) => {
    ...
    const errors = (validate.errors ?? []).map(({ instancePath, keyword, params }) => ({
      instancePath,
      keyword,
      params,
    }))
    return {
      ...
      executionClass: accepted ? 'studio-integration-required' : 'offline-schema-contract',
...
const validators = new WeakMap<Record<string, unknown>, ReturnType<typeof ajv.compile>>()
...
export function compileJourneyExample(item: InventoryJourney, actions: Set<string>) {
  const names = [
    ...(item.specification.match(/Actions candidates : (.*)/)?.[1] ?? '').matchAll(/`([^`]+)`/g),
  ].map(match => match[1])
  ...
  return {
    ...
    expectedBusinessResult: item.specification.match(/^- Contrôle : (.*)$/m)?.[1] ?? null,
