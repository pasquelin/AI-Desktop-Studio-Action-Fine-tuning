...
export function validateTemplates(source: Record<string, string>, translated: unknown): string[] {
  ...
  for (const [key, text] of Object.entries(source)) {
    ...
    const tokens = (input: string) => JSON.stringify((input.match(/\{[^{}]+\}/g) ?? []).sort())
    if (tokens(text) !== tokens(value)) errors.push(`Changed placeholders: ${key}`)
  }
  return errors
}
