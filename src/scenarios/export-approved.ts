...
function checkConversation(example: Example): void {
  const tools = new Map(
    example.tools.map(tool => [tool.function.name, ajv.compile(tool.function.parameters)]),
  )
  ...
  let hasCall = false
  if (
    !example.messages.some(message => message.role === 'user' && message.content?.trim()) ||
  ...
  for (const message of example.messages) {
    ...
    if (message.role === 'tool') {
      ...
      if (!message.tool_calls && !message.content?.trim()) throw new Error('Empty message')
    }
    for (const call of message.tool_calls ?? []) {
      const validate = tools.get(call.function.name)
      const args: unknown = JSON.parse(call.function.arguments)
      if (!validate?.(args) || seen.has(call.id)) throw new Error('Invalid tool call')
      seen.add(call.id)
      pending.add(call.id)
      hasCall = true
    }
...
export function prepareApprovedExport(input: unknown, context: unknown) {
  if (!valid(input))
    throw new Error(`Invalid or unreviewed export bundle: ${ajv.errorsText(valid.errors)}`)
  ...
  for (const example of input.examples) {
    ...
    if (example.review.contentHash !== contentHash || evidence.contentHash !== contentHash)
...
export async function publishApprovedExport(
  ...
  const identity = createHash('sha256').update(canonical(prepared)).digest('hex')
...
export function verifyRecordedConversation(example: unknown, report: unknown): void {
  if (!record(example) || !record(example.evidence)) throw new Error('Missing example evidence')
  const evidence = example.evidence
  if (
    !record(report) ||
  ...
  for (const key of ['runId', 'studioRevision', 'catalogueHash', 'scenarioHash', 'kind'])
