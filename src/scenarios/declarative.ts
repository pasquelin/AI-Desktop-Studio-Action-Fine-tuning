...
export function parseScenario(value: unknown): DeclarativeScenario {
  if (!valid(value))
    throw new Error(`Invalid scenario: ${JSON.stringify(valid.errors)}`);
...
function readPath(value: unknown, path: string): unknown {
  ...
  for (const part of path.split(".")) {
    ...
    if (
      (!record(value) && !Array.isArray(value)) ||
...
export async function resolveValue(
  ...
  if (Array.isArray(value))
    return Promise.all(
      value.map((item) => resolveValue(item, bindings, readFile)),
    );
  ...
  if ("$ref" in value) {
    ...
    if ("find" in value) {
      if (!Array.isArray(found) || !record(value.find))
        ...
        (item) =>
          ...
          Object.entries(criteria).every(
            ([key, expected]) =>
              ...
              isDeepStrictEqual(item[key], expected),
          ),
      );
      if (matches.length !== 1)
        throw new Error("Selector must match exactly one target");
      found = matches[0];
    }
    if ("pick" in value) {
...
export async function executeDeclarative(
  ...
  const { missing } = benchReadiness(
    scenario,
    adapter.availableRequirements ?? [],
  );
  ...
  for (const key of scenario.requiredBindings ?? [])
    if (!Object.hasOwn(variables, key))
      throw new Error(`Missing fixture binding: ${key}`);
  ...
  return runScenario(
    ...
    scenario.steps.map((step) => ({
      ...
      label: step.action,
      run: async () => {
        const input = await resolveValue(
          step.input,
          bindings,
          adapter.readFile,
        );
        ...
        if (step.expectRefusal && !refused)
          throw new Error("Expected refusal did not occur");
