...
async function session(root: string): Promise<string | undefined> {
  try {
    const value: unknown = JSON.parse(
      await readFile(join(folderFor(root), "session.json"), "utf8"),
    );
    if (
      !value ||
      typeof value !== "object" ||
...
export async function beginSnapshots(root: string, run: string): Promise<void> {
  ...
  for (const file of await readdir(folder))
    if (
      SNAPSHOT.test(file) ||
      (file.endsWith(".json") && SNAPSHOT.test(file.slice(0, -5)))
    )
...
export async function saveSnapshot(
  ...
  if (!current && (await latestRun(root))?.name === run)
    await beginSnapshots(root, run);
  if ((await session(root)) !== run)
    throw new Error("Capture belongs to an older session");
  if (
...
export async function readSnapshot(
  ...
  if (!SNAPSHOT.test(file) || (await session(root)) !== run)
