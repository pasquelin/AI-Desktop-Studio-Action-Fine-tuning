...
export function scenarioConsent(action: string, result: unknown): string | undefined {
  if (
    !actions.has(action) ||
    !result ||
  ...
  const text = result.content
    ...
    .map(item => item.text)
    .join('\n')
  if (!text.startsWith('Nothing engaged: that call needs a consent token.')) return undefined
