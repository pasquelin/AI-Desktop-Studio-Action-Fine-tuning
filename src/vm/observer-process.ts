...
export const OBSERVER_URL = "http://127.0.0.1:4318/";
async function runningObserver(root: string): Promise<boolean> {
  try {
    ...
    return (
      response.ok &&
      value.service === "studio-vm-observer" &&
      value.root === root
    );
  } catch {
    return false;
...
export async function ensureObserver(root: string): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(
      () => fail(new Error("Observer did not start")),
      10000,
    );
    child.once("error", fail);
    child.once("exit", () =>
      fail(new Error("Observer exited before publishing its address")),
    );
