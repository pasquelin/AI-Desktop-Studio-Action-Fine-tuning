...
export function inspectSource(root: string, expectedRevision?: string) {
  ...
  const source = readFileSync(
    join(root, "src/shared/domain/assistantActionNames.ts"),
    "utf8",
  );
  // Fail closed if Studio changes this explicit literal-union contract.
  if (
    !/^export type ActionName\s*=\s*(?:\|\s*'[^']+'\s*)+;?\s*$/.test(source)
  ) {
...
export function assertFresh(
  ...
  if (
    ...
    Object.keys(previous.sourceHashes).length !==
      Object.keys(current.hashes).length ||
    Object.entries(current.hashes).some(
      ([file, hash]) => previous.sourceHashes[file] !== hash,
    )
  ) {
    throw new Error(
      "Catalogue is stale: run catalogue:export to regenerate it from Studio.",
    );
  }
}
