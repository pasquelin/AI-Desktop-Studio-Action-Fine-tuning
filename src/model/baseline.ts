...
export const ENDPOINT = "http://127.0.0.1:11434";
...
export function readTools(catalogue: unknown, names = TOOL_NAMES): Tool[] {
  ...
  return names.map((name) => {
    const matches = entries.filter(
      (item) => record(item) && item.name === name,
    );
...
export function grade(value: unknown, expected: Proposal, tools: Tool[]) {
  ...
  if (
    !new Ajv({ strict: false }).validate(tool.inputSchema, proposed.arguments)
  ) {
    return { passed: false, reason: "Arguments violate the Studio schema" };
  }
  const passed =
    proposed.name === expected.name &&
    isDeepStrictEqual(proposed.arguments, expected.arguments);
  return {
    passed,
    reason: passed
