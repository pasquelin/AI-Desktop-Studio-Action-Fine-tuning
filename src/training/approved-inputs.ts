...
import { assertCurrentJourneySources } from '../admin/current-sources.ts'
import { loadQaStatusIndex } from '../admin/scenario-qa-state.ts'
import { sha256 } from '../catalogue/catalogue.ts'
import { assertFresh, inspectSource } from '../catalogue/source.ts'
import { record } from '../json.ts'
import { prepareApprovedExport, verifyRecordedConversation } from '../scenarios/export-approved.ts'
import { readStudioLink } from '../studio/checkout.ts'
import { readRecord, stateDir } from '../vm/ownership.ts'
import { assertQaEligibleExamples } from './qa-eligibility.ts'
...
export async function approvedInputs(root: string, bundleFile: string, scenarioManifest: string) {
  ...
  assertQaEligibleExamples(bundle, await loadQaStatusIndex(root))
