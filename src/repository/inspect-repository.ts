...
export function inspectRepository(root: string, maxBytes = 1_048_576): string[] {
  ...
  for (const name of new Set(names.split('\0').filter(Boolean))) {
    ...
    if (stat.isSymbolicLink()) errors.push(`${name}: symbolic link is not allowed.`)
    else if (!stat.isFile()) errors.push(`${name}: only regular files are allowed.`)
    else if (stat.size > maxBytes) errors.push(`${name}: exceeds ${maxBytes} bytes.`)
  }
  return errors
}
