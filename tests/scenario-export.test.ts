...
describe("approved export (synthetic evidence only)", () => {
  ...
  it("rejects changed content, Studio, catalogue and scenario hashes", () => {
    ...
    expect(() =>
      prepareApprovedExport(fixture(), { ...context, studioRevision: "new" }),
  ...
  it("rejects a requested partition that conflicts with its semantic family", () => {
    const bundle = fixture();
    const split =
      splitFor("node.add", bundle.seed) === "train" ? "test" : "train";
    expect(() =>
      prepareApprovedExport(
        { ...bundle, examples: [{ ...bundle.examples[0], split }] },
        context,
      ),
  ...
  it("rejects identical conversations assigned to different semantic groups", () => {
    ...
    expect(() =>
      prepareApprovedExport(
        { ...bundle, examples: [...bundle.examples, second] },
        context,
      ),
  ...
  it("requires a real saved conversation link, not copied evidence fields", () => {
    ...
    expect(() => verifyRecordedConversation(example, report)).toThrow(
      "no evidence linked",
    );
    const linked = {
      ...report,
    ...
    expect(() =>
      verifyRecordedConversation(example, { ...linked, status: "failed" }),
  ...
  it("publishes all files together and leaves no directory after rejection", async () => {
    ...
    try {
      ...
      expect(
        (await readdir(parent)).some((name) => name.startsWith(".approved-")),
      ).toBe(false);
    } finally {
