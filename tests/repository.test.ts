...
describe("repository hygiene", () => {
  it("accepts small source files including names with spaces", () => {
    writeFileSync(
      join(root, "source with spaces.ts"),
  ...
  it.each([
    ...
    "id_rsa",
    "secret.pem",
  ])("rejects distributable files matching %s", (name) => {
    writeFileSync(join(root, name), "not a real secret or model");
    expect(inspectRepository(root)).toContainEqual(
  ...
  it.skipIf(process.platform === "win32")(
    "rejects symlinks without reading the target",
    () => {
