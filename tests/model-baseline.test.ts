...
it("rejects unknown parameters and tools", () => {
  ...
  expect(
    grade(response({ kind: "box", consent: "invented" }), expected, tools)
      .passed,
  ).toBe(false);
  expect(
    grade(response({ kind: "box" }, "unknown"), expected, tools).passed,
  ).toBe(false);
});
it("rejects prose or malformed output instead of treating it as an action", () => {
  for (const value of [
    ...
    expect(
      grade(value, expected, readTools(catalogue, ["node_add"])).passed,
    ).toBe(false);
  }
});
...
it("requires every selected tool to exist once and have a schema", () => {
  ...
  expect(() =>
    readTools({ mcpTools: [...catalogue.mcpTools, ...catalogue.mcpTools] }, [
      "node_add",
    ]),
  ).toThrow();
  expect(() =>
    readTools({ mcpTools: [{ name: "node_add" }] }, ["node_add"]),
  ).toThrow();
});
