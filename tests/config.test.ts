...
const fixture = (): Record<string, unknown> =>
  JSON.parse(readFileSync(new URL('../configs/pilot.json', import.meta.url), 'utf8'))
...
describe('pilot configuration boundary', () => {
  ...
  it('rejects duplicate model identifiers even if roles differ', () => {
    const models = [model('test/same', 'primary'), model('test/same', 'comparison')]
    expect(validatePilot({ ...fixture(), models })).toContain('Model identifiers must be unique.')
  })
