  ...
  steps: [
    {
      ...
      saveAs: "scene",
      assertions: [
        { actual: { $ref: "scene.nodes" }, op: "length", expected: 0 },
      ],
    },
  ],
});
...
const calls = [
  { stepId: "read", action: "scene.state", input: {}, result: { nodes: [] } },
];
it("links exact recorded conversation but keeps semantic approval pending", () => {
  ...
  expect(() =>
    prepareApprovedExport({ seed: "seed", examples: [draft] }, {}),
  ).toThrow("unreviewed");
  expect(() =>
    verifyRecordedConversation(
      {
...
it("never creates a draft from incomplete or wrongly ordered execution", () => {
  expect(() =>
    makeConversationDraft(scenario, report, [], tools, provenance),
  ).toThrow("complete");
  expect(() =>
...
it("only an actual typed refusal can satisfy a negative scenario", async () => {
  ...
  expect((await execute(async () => ({ refused: true }))).status).toBe(
    "failed",
  );
  expect(
    (
