...
const tools = (schema: unknown = parameters) => [
  { name: "node_add", inputSchema: schema },
];
...
describe("scenario input preflight", () => {
  it("accepts valid literals", () =>
    expect(() =>
      checkScenarioInputs(
        plan({ name: "Cube", config: { count: 1, enabled: true } }),
        tools(),
      ),
    ).not.toThrow());
  it("rejects unknown actions, fields and missing fields", () => {
    expect(() =>
      checkScenarioInputs(plan({}, "missing.action"), tools()),
    ...
    expect(() => checkScenarioInputs(plan({ name: "Cube" }), tools())).toThrow(
      "required property",
    );
  });
  ...
  it("defers file reads while retaining missing nested-field checks", () => {
    ...
    expect(() =>
      checkScenarioInputs(
        plan({ name: "Cube", config: { count: { $ref: "counter" } } }),
        tools(),
      ),
    ).toThrow("enabled");
  });
  it("resolves root-local schema references and refuses unavailable ones", () => {
    ...
    expect(() =>
      checkScenarioInputs(
        plan({ name: 1, config: { count: 1, enabled: true } }),
        tools(schema),
      ),
    ).toThrow(">= 2");
    expect(() =>
      checkScenarioInputs(
        plan({}),
        tools({ $ref: "https://unavailable.invalid/schema" }),
      ),
  ...
  it("does not certify an unresolved conditional branch", () => {
    ...
    expect(() =>
      checkScenarioInputs(plan({ name: { $ref: "choice" } }), tools(schema)),
    ).toThrow("conditional schema");
  });
});
