    ...
    new Set(
      await Promise.all(
        items.map(async (item) =>
          (await readSnapshot(root, item.run, item.file)).toString(),
        ),
      ),
    ),
...
it("rejects path traversal and unrelated files", async () => {
  ...
  await expect(
    readSnapshot(root, "../../outside", "observation.jpg"),
  ).rejects.toThrow();
...
it("freezes the recorded stage with the capture", async () => {
  ...
  expect((await listSnapshots(root))[0]?.activity).toBe(
    "Construction de Studio",
  );
});
