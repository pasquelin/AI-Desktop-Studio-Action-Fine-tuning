...
describe('catalogue export contract', () => {
  ...
  it('detects a changed field in the catalogue hash', () => {
    ...
    const after = makeCatalogue(
      {
        ...changed,
        actions: [{ ...changed.actions[0], fields: [{ key: 'name', required: true }] }],
      },
      ['node.add'],
      revision,
      {},
  ...
  it('rejects a declared action absent from the registry', () => {
    expect(() => makeCatalogue(snapshot(), ['node.add', 'node.remove'], revision, {})).toThrow(
      /declared/i,
    )
  })
  ...
  it('rejects family order inconsistent with the registry', () => {
    expect(() =>
      makeCatalogue({ ...snapshot(), registryNames: [] }, ['node.add'], revision, {}),
    ).toThrow(/registry/i)
  })
